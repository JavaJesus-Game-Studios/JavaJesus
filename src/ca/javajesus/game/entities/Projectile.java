...
public class Projectile extends Particle {
    ...
    public Projectile(Level level, int width, int height, int tileNumber,
            int color, double x, double y, int speed, int direction) {
    ...
    private void calcSlope(double x, double y) {
        this.yPoint = y - this.y;
        this.xPoint = x - this.x;
    }
    ...
    public void render(Screen screen) {
        ...
        this.y += speed * yPoint / 50;
        this.x += speed * xPoint / 50;

        screen.render(this.x, this.y, tileNumber + (yOffset * 32), color, 1, 1, sheet);
