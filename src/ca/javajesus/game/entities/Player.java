...
public class Player extends Mob {
    ...
    public void tick() {
        ...
        if (swingTickCount % 60 <= 45 && isSwinging) {
            swingTick = 2;
        }
        ...
        // When does the cooldown get triggered? The player cannot start another
        // attack animation unless this amount of time has passed
        if (tickCount % 60 == 0 && isSwinging) {
            cooldown = false;
        } else if (tickCount % 60 == 0 && isShooting) {
            cooldown = false;
        } else {
            cooldown = true;
        }
    ...
    public void render(Screen screen) {
        ...
        // Determines the animation position at the designated tick
        switch (swingTick) {
        case 1:
            swingModifier = 3;
            break;
        ...
        if (isShooting && !isSwimming) {
            ...
            screen.render(xOffset + modifier - (modifier * flipAttack1),
                    yOffset, (xTile + 1 + shootModifier) + (yTile + 2) * 32,
                    colour, flipAttack1, scale, sheet);

            // Lower Body 1
            screen.render(xOffset + (modifier * flipAttack2), yOffset
                    + modifier, xTile + (yTile + 3) * 32, colour, flipAttack2,
                    scale, sheet);

            // Lower Body 2
            screen.render(xOffset + modifier - (modifier * flipAttack2),
                    yOffset + modifier, (xTile + 1 + shootModifier)
                            + (yTile + 3) * 32, colour, flipAttack2, scale,
                    sheet);
