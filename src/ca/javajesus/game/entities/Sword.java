...
public class Sword extends Entity {
	...
	protected SpriteSheet sheet = new SpriteSheet("/Swords/GreatSword_Sheet.png");
	...
	private void getImage() {
		try {
			this.image = ImageIO.read(new File(
					"res/Swords/GreatSword_Sheet.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void tick() {
		...
		angle++;
		/*
		 * if (angle < 0) { angle = 90; level.remEntity(this); }
		 */
	 ...
	 * 
	 * g2d.setColor(Color.RED); // g2d.drawLine(Game.WIDTH * Game.SCALE / 2,
	 * Game.HEIGHT * Game.SCALE / 2, p.x, p.y);
	 * 
	 * AffineTransform at = AffineTransform.getTranslateInstance(p.x, p.y);
	 * 
	 * at.rotate(Math.toRadians(-angle)); g2d.setTransform(at);
	 * 
	 * switch(player.movingDir) { case 2: { g2d.drawImage(image, 0, 0, width,
	 * height, null); break; } case 3: { g2d.drawImage(image, 0, 0, -width,
	 * height, null); break; } } g2d.dispose(); }
	 * 
	 * public void renderSwordPlayer(Screen screen) { player =
	 * screen.getGame().player; return; }
	 */
	protected Point getSwordHandlePoint() {
		...
		int xPos = Math.round((float) ((x + Math.cos(rads) * fullLength)) / 15
				+ Game.WIDTH * Game.SCALE / 2 + 20);
		int yPos = Math.round((float) ((y - Math.sin(rads) * fullLength)) / 15
				+ Game.HEIGHT * Game.SCALE / 2 - 10);
