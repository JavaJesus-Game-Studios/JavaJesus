...
public class LordHillsborough extends NPC {
	...
	public void doDialogue() {
		...
		case 4: {
			ChatHandler.displayText(getName() + ": Uggh you bore me with your trifling!", Color.white);
			return;
		}
		case 5: {
			ChatHandler.displayText(getName() + ": Hello, salutations, and welcome to my domain Officer!", Color.white);
			return;
		}
		case 6: {
			ChatHandler.displayText(getName() + ": They may be peasants, but they are my peasants, and I'll be damned"
					+ "if some bow-legged, red-skinned devil men try to take them from me!", Color.white);
			return;
		}
		case 7: {
			ChatHandler.displayText(getName() + ": Oh how I loathe the cities and their lack royalty, how is one to k"
					+ "now if he is better than everyone else without a title" + "to prove it?", Color.white);
			return;
		}
		case 8: {
			ChatHandler.displayText(getName() + ": Manners maketh man.", Color.white);
			return;
		}
		case 9: {
			ChatHandler.displayText(getName() + ": I'm am racist so when you're driving in the East Bay,"
		...
		case 12: {
			ChatHandler.displayText(getName() + ": Oh how I miss the elder days that father spoke of, let me tell you"
		...
		default: {
			ChatHandler.displayText(getName() + ": A gentlemen's weapon is the sabre, these 'firearms' that "
