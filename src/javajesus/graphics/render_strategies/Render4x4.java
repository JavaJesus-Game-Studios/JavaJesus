...
public class Render4x4 implements IRenderStrategy {
	...
	public void renderSwimming(Screen screen, int xLocation, int yLocation,
			...
			int[] color) {
		// Left Tile
		screen.render(xLocation + (TILE_SIZE * (flip ? 1: 0)), yLocation + 3 * TILE_SIZE, xTile,
				yTile, sheet, flip, color);
		// Middle Left Tile
		screen.render(xLocation + (TILE_SIZE * (flip ? 0: 1)), yLocation + 3 * TILE_SIZE, xTile + 1,
				yTile, sheet, flip, color);
		// Middle Right Tile
		screen.render(xLocation + (TILE_SIZE * (flip ? 0: 2)), yLocation + 3 * TILE_SIZE, xTile + 2,
				yTile, sheet, flip, color);
		// Right Tile
		screen.render(xLocation + (TILE_SIZE * (flip ? 0: 3)), yLocation + 3 * TILE_SIZE, xTile + 3,
				yTile, sheet, flip, color);
	}
