...
public class Render3x3 implements IRenderStrategy {
	...
	public void renderSwimming(Screen screen, int xLocation, int yLocation,
			...
			int[] color) {
		// Left
		screen.render(xLocation + (TILE_SIZE * (flip ? 1: 0)), yLocation + 2 * TILE_SIZE, xTile, yTile,
				sheet, flip, color);
		// Middle
		screen.render(xLocation + (TILE_SIZE * (flip ? 0: 1)), yLocation + 2 * TILE_SIZE, xTile + 1, yTile,
				sheet, flip, color);
		// Right
		screen.render(xLocation + (TILE_SIZE * (flip ? 0: 2)), yLocation + 2 * TILE_SIZE, xTile + 2, yTile,
				sheet, flip, color);
	}
