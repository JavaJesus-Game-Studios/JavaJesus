...
public class Screen {
	...
	public void render(int xOffset, int yOffset, int tile, int[] color,
		...
		int tileOffset = (xTile << Tile.TILE_SHIFT) + (yTile << Tile.TILE_SHIFT) * sheet.getWidth();
		for (int y = 0; y < Tile.SIZE; y++) {
			...
			int yPixel = y + yOffset + (y * scaleMap) - ((scaleMap << Tile.TILE_SHIFT) >> 1);
			for (int x = 0; x < Tile.SIZE; x++) {
				int xPixel = x + xOffset + (x * scaleMap)
						- ((scaleMap << Tile.TILE_SHIFT) >> 1);
				int xSheet = x;
	...
	/**
	 * @return width of the pixels on the screen
	 */
	public int getWidth() {
		return width;
	}
	...
	/**
	 * @return height of the pixels on the screen
	 */
