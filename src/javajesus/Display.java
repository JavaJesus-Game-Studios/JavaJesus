...
public class Display extends Canvas {
	...
	// Frame)
	public static final int IMAGE_WIDTH = 225 * 12 / 9 * 4;

	// Game Screen height (Size of the In-Game Map displayed within the Actual
	// Frame)
	public static final int IMAGE_HEIGHT = 225 * 4;
	 ...
	 */
	public void createPlayer(String name, int shirtColor, int skinColor) {
	...
	protected void render() {
		...
		int xOffset = player.getX() - (screen.getWidth() >> 1);
		int yOffset = player.getY() - (screen.getHeight() >> 1);
		if (player.isDriving()) {
			xOffset = player.getVehicle().getX() - (screen.getWidth() >> 1);
			yOffset = player.getVehicle().getY() - (screen.getHeight() >> 1);
		}
